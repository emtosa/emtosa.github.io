...
- **If neither is possible, restart the job after cert renewal.** This is the operational workaround: schedule a job restart shortly after the cert renewal window, so the job picks up the new cert before the old one expires. Not elegant, but it's the documented-pattern fallback when the credential can't be reloaded mid-flight.
